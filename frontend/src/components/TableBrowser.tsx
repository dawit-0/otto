...
// Sentinel used by DataTable to signal "set to NULL"
...
export default function TableBrowser({ dbId, tableName, columnDefs }: Props) {
  ...
  const handleCellSave = useCallback(async (
    ...
    const pkVals = pkColumns.map((pk) => row[pk]);
    const value = newValue === NULL_SENTINEL ? null : newValue;
    await api.updateCell(dbId, tableName, pkColumns, pkVals, colName, value);
  ...
  const handleRowDelete = useCallback(async (row: Record<string, unknown>) => {
    const pkVals = pkColumns.map((pk) => row[pk]);
    await api.deleteRow(dbId, tableName, pkColumns, pkVals);
    await loadData(offset, sort, filters);
  }, [dbId, tableName, pkColumns, offset, sort, filters, loadData]);

  const handleInsertRow = useCallback(async (data: Record<string, unknown>) => {
          ...
          <div className="filter-toolbar">
            <div className="filter-toolbar-controls">
              ...
              <div className="filter-toolbar-right">
                ...
                {/* Edit mode toggle */}
                <button
                  className={`btn btn-sm btn-edit-toggle${editMode ? ' btn-edit-toggle-active' : ''}`}
                  onClick={() => setEditMode((v) => !v)}
                  title={hasPk ? (editMode ? 'Exit edit mode' : 'Enter edit mode to modify data') : 'No primary key — editing unavailable'}
                  disabled={!hasPk}
                >
