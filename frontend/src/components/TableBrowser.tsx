...
export default function TableBrowser({ dbId, tableName, columnDefs }: Props) {
  ...
  const hasPK = columnDefs.some((c) => c.pk);
  ...
  // Reset edit mode when table changes
  useEffect(() => {
    setEditMode(false);
  }, [dbId, tableName]);
  ...
  const handleDataChange = useCallback(() => {
    // Refresh the current page with current sort/filter after a successful save
        ...
        <div className="filter-toolbar-controls">
          ...
          <div className="filter-toolbar-right">
            ...
            <button
              ...
              title={
                !hasPK
                  ? 'This table has no primary key — editing is unavailable'
                  : editMode
                  ? 'Exit edit mode'
                  : 'Edit table rows'
              }
            >
              {editMode ? (
                <>
                  <span className="edit-mode-dot-sm" />
                  Editing
                </>
              ) : (
                <>
