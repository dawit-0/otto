...
export default function TableBrowser({ dbId, tableName, columnDefs }: Props) {
  ...
  const handleUpdateRow = async (pk: Record<string, unknown>, updates: Record<string, unknown>) => {
    await api.updateRow(dbId, tableName, pk, updates);
    ...
    <div className={`table-browser-wrapper${showProfile ? ' profile-open' : ''}`}>
      ...
      {!error && (
        <DataTable
          ...
          onSort={handleSort}
          pkColumns={pkColumns}
          onUpdateRow={pkColumns.length > 0 ? handleUpdateRow : undefined}
          onInsertRow={handleInsertRow}
          onDeleteRow={pkColumns.length > 0 ? handleDeleteRow : undefined}
        />
      )}
      </div>
