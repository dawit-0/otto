...
def update_table_row(db_id: str, table_name: str, req: RowUpdateRequest, db: Session = Depends(get_db)):
    ...
    try:
        driver.assert_valid_table(conn, table_name)
        row = driver.update_row(conn, table_name, req.row_id, req.values)
        logger.info("Updated row in '%s'.'%s'", db_name, table_name)
        return {"row": row}
    except ValueError as e:
        status = 404 if str(e).lower().startswith("unknown table") or str(e).lower() == "row not found" else 400
        raise HTTPException(status_code=status, detail=str(e))
    except Exception as e:
...
def delete_table_row(db_id: str, table_name: str, req: RowDeleteRequest, db: Session = Depends(get_db)):
    ...
    try:
        driver.assert_valid_table(conn, table_name)
        driver.delete_row(conn, table_name, req.row_id)
